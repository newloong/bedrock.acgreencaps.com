...
use Barn2\Plugin\Discount_Manager\Dependencies\Barn2\Databasepress\SQL\Select;
...
use Barn2\Plugin\Discount_Manager\Dependencies\Barn2\DatabasepressORM\IDataMapper;
...
class DataMapper implements IDataMapper
{
    ...
    {
        $this->linkOrUnlink($relation, $entity, \true);
    }
    /**
     * @param string $relation
     * @param Entity $entity
    ...
    protected function hydrate()
    {
        ...
        foreach ($this->mapper->getPrimaryKey()->getValue($this->rawColumns, \true) as $pk_column => $pk_value) {
            $select->where($pk_column)->is($pk_value);
        }
        $columns = $select->select()->first();
        if ($columns === \false) {
            $this->deleted = \true;
            return;
        }
    ...
    private function linkOrUnlink(string $relation, Entity $entity, bool $link)
    {
        ...
        $this->pendingLinks[] = ['relation' => $rel, 'entity' => $entity, 'link' => $link];
    }
}
