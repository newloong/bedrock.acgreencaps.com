...
class Settings_API_Helper implements Registerable, Conditional
{
    ...
    private static function field_description($args)
    {
        if (!empty($args['desc'])) {
            ...
            if ($is_page_select) {
                $args['desc'] .= ' <a href="' . \esc_url(\get_permalink($args['show_page'])) . '" target="_blank">' . esc_html__('[View page]', 'woocommerce-discount-manager') . '</a>';
            }
    ...
    public static function settings_field_color_size($args)
    {
        ...
        $size_placeholder = !empty($args['placeholder']) ? $args['placeholder'] : __('Size', 'woocommerce-discount-manager');
        if (empty($args['custom_attributes'])) {
