...
use Barn2\Plugin\Discount_Manager\Dependencies\Lib\Plugin\License\License_API;
...
class Plugin_Updater implements Registerable
{
    ...
    public function update_available_notice($plugin_data, $response)
    {
        ...
        if (empty($response->package)) {
            ...
            \printf(' <em>%s</em>', \sprintf(
                ...
                __('Activate %1$syour license key%2$s to enable updates.', 'woocommerce-discount-manager'),
                $settings_link_open,
                $settings_link_close
            ));
    ...
    function auto_update_setting_html($html, $plugin_file)
    {
        ...
        if (!$this->plugin->get_license()->is_valid() || !\apply_filters('barn2_plugin_allow_automatic_update', \true, $this->plugin)) {
            $html = \sprintf('<em>%s</em>', __('Auto-updates unavailable.', 'woocommerce-discount-manager'));
        }
        return $html;
    }
    /**
