...
use Barn2\Plugin\Discount_Manager\Dependencies\Setup_Wizard\Interfaces\Deferrable;
...
class Api implements JsonSerializable
{
    ...
    public function save_fields($request)
    {
        ...
        if (empty($step) || !$step instanceof Step) {
            return self::send_error_response(['message' => __('Could not find the appropriate step.', 'woocommerce-discount-manager')]);
        }
    ...
    public function handle_license($request)
    {
        ...
        if (empty($license_key)) {
            return self::send_error_response(['message' => __('Please enter a license key.', 'woocommerce-discount-manager')]);
        }
        if (!\in_array($action, $allowed_actions, \true)) {
            return self::send_error_response(['message' => __('Invalid action requested.', 'woocommerce-discount-manager')]);
        }
        switch ($action) {
            case 'activate':
