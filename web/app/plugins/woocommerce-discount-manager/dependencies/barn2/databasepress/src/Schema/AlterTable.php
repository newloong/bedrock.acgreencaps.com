...
class AlterTable
{
    ...
    {
        $this->commands[] = ['type' => $name, 'data' => $data];
        return $this;
    }
    /**
     * @param string          $type
    ...
    {
        static $map = ['addPrimary' => 'pk', 'addUnique' => 'uk', 'addForeignKey' => 'fk', 'addIndex' => 'ik'];
        if (!\is_array($columns)) {
            $columns = [$columns];
        }
        if ($name === null) {
        ...
        return $this->addCommand($type, ['name' => $name, 'columns' => $columns]);
    }
    /**
     * @param string $name
     * @param string $type
    ...
    {
        return $this->addCommand('renameColumn', ['from' => $from, 'column' => new AlterColumn($this, $to)]);
    }
    /**
     * @param string|string[] $columns
     * @param string|null     $name
    ...
    public function foreign($columns, string $name = null) : ForeignKey
    {
        ...
        $this->addCommand('addForeign', ['name' => $name, 'foreign' => $foreign]);
        return $foreign;
    }
    /**
     * @param string $column
    ...
    {
        return $this->addCommand('setDefaultValue', ['column' => $column, 'value' => $value]);
    }
    /**
     * @param string $name
     * @return AlterColumn
