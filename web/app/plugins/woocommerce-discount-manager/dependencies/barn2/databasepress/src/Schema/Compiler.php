...
use Barn2\Plugin\Discount_Manager\Dependencies\Barn2\Databasepress\Connection;
...
class Compiler
{
    ...
    protected function handleColumnModifiers(BaseColumn $column) : string
    {
        ...
        foreach ($this->modifiers as $modifier) {
            ...
            $result = \trim($this->{$callback}($column));
            if ($result !== '') {
                $result = ' ' . $result;
            }
            $line .= $result;
     ...
     * @return string
     */
    protected function handleCharacterSet(CreateTable $schema) : string
    {
        global $wpdb;
        if ($wpdb) {
    ...
    {
        return 'ALTER TABLE ' . $this->wrap($table->getTableName()) . ' ADD CONSTRAINT ' . $this->wrap($data['name']) . ' PRIMARY KEY (' . $this->wrapArray($data['columns']) . ')';
    }
    /**
     * @param AlterTable $table
     * @param mixed      $data
    ...
    {
        return 'ALTER TABLE ' . $this->wrap($table->getTableName()) . ' ADD CONSTRAINT ' . $this->wrap($data['name']) . ' UNIQUE (' . $this->wrapArray($data['columns']) . ')';
    }
    /**
     * @param AlterTable $table
     * @param mixed      $data
    ...
    protected function handleAddForeign(AlterTable $table, $data) : string
    {
        /** @var ForeignKey $key */
        $key = $data['foreign'];
        return 'ALTER TABLE ' . $this->wrap($table->getTableName()) . ' ADD CONSTRAINT ' . $this->wrap($data['name']) . ' FOREIGN KEY (' . $this->wrapArray($key->getColumns()) . ') ' . 'REFERENCES ' . $this->wrap($key->getReferencedTable()) . '(' . $this->wrapArray($key->getReferencedColumns()) . ')';
    }
    /**
     * @param AlterTable $table
     * @param mixed      $data
    ...
    {
        return 'ALTER TABLE ' . $this->wrap($table->getTableName()) . ' ALTER COLUMN ' . $this->wrap($data['column']) . ' SET DEFAULT ' . $this->value($data['value']);
    }
    /**
     * @param AlterTable $table
     * @param mixed      $data
    ...
    {
        return 'ALTER TABLE ' . $this->wrap($table->getTableName()) . ' ALTER COLUMN ' . $this->wrap($data) . ' DROP DEFAULT';
    }
    /**
     * @return array
     */
    ...
    {
        $sql = 'SELECT ' . $this->wrap('table_name') . ' FROM ' . $this->wrap('information_schema') . '.' . $this->wrap('tables') . ' WHERE table_type = ? AND table_schema = ? ORDER BY ' . $this->wrap('table_name') . ' ASC';
    ...
    {
        $sql = 'SELECT ' . $this->wrap('column_name') . ' AS ' . $this->wrap('name') . ', ' . $this->wrap('column_type') . ' AS ' . $this->wrap('type') . ' FROM ' . $this->wrap('information_schema') . '.' . $this->wrap('columns') . ' WHERE ' . $this->wrap('table_schema') . ' = ? AND ' . $this->wrap('table_name') . ' = ? ' . ' ORDER BY ' . $this->wrap('ordinal_position') . ' ASC';
    ...
    public function create(CreateTable $schema) : array
    {
        ...
        $sql .= "\n)" . $this->handleEngine($schema) . $this->handleCharacterSet($schema);
        $commands = [];
