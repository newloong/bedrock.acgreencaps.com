...
class SQLStatement
{
    ...
    public function addWhereConditionGroup(Closure $callback, $separator)
    {
        ...
        $callback($where);
        $this->wheres[] = ['type' => 'whereNested', 'clause' => $where->getSQLStatement()->getWheres(), 'separator' => $separator];
    }
    /**
    ...
    {
        $this->wheres[] = ['type' => 'whereColumn', 'column' => $this->closureToExpression($column), 'value' => $this->closureToExpression($value), 'operator' => $operator, 'separator' => $separator];
    }
    /**
     * @param string|Closure|Expression $column
    ...
    {
        $this->wheres[] = ['type' => 'whereLike', 'column' => $this->closureToExpression($column), 'pattern' => $pattern, 'separator' => $separator, 'not' => $not];
    }
    /**
     * @param string|Closure|Expression $column
     * @param $value1
    ...
    {
        $this->wheres[] = ['type' => 'whereBetween', 'column' => $this->closureToExpression($column), 'value1' => $this->closureToExpression($value1), 'value2' => $this->closureToExpression($value2), 'separator' => $separator, 'not' => $not];
    }
    /**
     * @param string|Closure|Expression $column
     * @param $value
    ...
    public function addWhereInCondition($column, $value, string $separator, bool $not)
    {
        ...
        if ($value instanceof Closure) {
            $select = new Subquery();
            $value($select);
            $this->wheres[] = ['type' => 'whereInSelect', 'column' => $column, 'subquery' => $select, 'separator' => $separator, 'not' => $not];
        } else {
            $this->wheres[] = ['type' => 'whereIn', 'column' => $column, 'value' => $value, 'separator' => $separator, 'not' => $not];
        }
    }
    /**
     * @param string|Closure|Expression $column
    ...
    {
        $this->wheres[] = ['type' => 'whereNull', 'column' => $this->closureToExpression($column), 'separator' => $separator, 'not' => $not];
    }
    /**
     * @param string|Closure|Expression $column
     * @param string                    $separator
    ...
    {
        $this->wheres[] = ['type' => 'whereNop', 'column' => $column, 'separator' => $separator];
    }
    /**
    ...
    public function addWhereExistsCondition(Closure $closure, string $separator, bool $not)
    {
        $select = new Subquery();
        $closure($select);
        $this->wheres[] = ['type' => 'whereExists', 'subquery' => $select, 'separator' => $separator, 'not' => $not];
    }
    /**
     * @param  string       $type
    ...
    public function addJoinClause(string $type, $table, Closure $closure = null)
    {
        ...
        $this->joins[] = ['type' => $type, 'table' => $table, 'join' => $join];
    }
    /**
     * @param   Closure $callback
     * @param   string  $separator
    ...
    public function addHavingGroupCondition(Closure $callback, string $separator)
    {
        ...
        $callback($having);
        $this->having[] = ['type' => 'havingNested', 'conditions' => $having->getSQLStatement()->getHaving(), 'separator' => $separator];
    }
    /**
    ...
    {
        $this->having[] = ['type' => 'havingCondition', 'aggregate' => $this->closureToExpression($aggregate), 'value' => $this->closureToExpression($value), 'operator' => $operator, 'separator' => $separator];
    }
    /**
     * @param   string|Closure|Expression $aggregate
     * @param   mixed                     $value
    ...
    public function addHavingInCondition($aggregate, $value, string $separator, bool $not)
    {
        ...
        if ($value instanceof Closure) {
            $select = new Subquery();
            $value($select);
            $this->having[] = ['type' => 'havingInSelect', 'aggregate' => $aggregate, 'subquery' => $select, 'separator' => $separator, 'not' => $not];
        } else {
            $this->having[] = ['type' => 'havingIn', 'aggregate' => $aggregate, 'value' => $value, 'separator' => $separator, 'not' => $not];
        }
    }
    /**
     * @param   string|Closure|Expression $aggregate
    ...
    {
        $this->having[] = ['type' => 'havingBetween', 'aggregate' => $this->closureToExpression($aggregate), 'value1' => $this->closureToExpression($value1), 'value2' => $this->closureToExpression($value2), 'separator' => $separator, 'not' => $not];
    }
    /**
     * @param array $tables
     */
    ...
    public function addUpdateColumns(array $columns)
    {
        foreach ($columns as $column => $value) {
            $this->columns[] = ['column' => $column, 'value' => $this->closureToExpression($value)];
        }
    }
    /**
    ...
    public function addOrder(array $columns, string $order, string $nulls = null)
    {
        ...
        $this->order[] = ['columns' => $columns, 'order' => $order, 'nulls' => $nulls];
    }
    /**
     * @param string[]|Expression[]|Closure[] $columns
     */
    ...
    {
        $this->columns[] = ['name' => $this->closureToExpression($column), 'alias' => $alias];
    }
    /**
     * @param bool $value
     */
