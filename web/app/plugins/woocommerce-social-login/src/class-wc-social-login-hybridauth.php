...
class WC_Social_Login_HybridAuth {
	 ...
	 *
	 * Initializes HybridAuth configuration with the configured strategies.
	 * HybridAuth will be instantiated separately in the authentication and callback methods,
	 * because HybridAuth will try to create authentication request instantly when instantiated.
	 *
	 * @internal
	 *
	 * @since 2.0.0
	...
	public function init_config() {
		...
		if ( Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte( '8.6' ) ) {
			$directory  = trailingslashit( realpath( \Automattic\Jetpack\Constants::get_constant( 'WC_LOG_DIR' ) ) );
			$file_id    = \Automattic\WooCommerce\Utilities\LoggingUtil::generate_log_file_id( wc_social_login()->get_id() . '_hybridauth', null, time() );
			$hash       = \Automattic\WooCommerce\Utilities\LoggingUtil::generate_log_file_hash( $file_id );
			$debug_file = "{$directory}{$file_id}-{$hash}.log";
		} else {
