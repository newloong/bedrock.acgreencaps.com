...
class NewBadge extends WooBase {

	public function get_name() {
		return 'reycore-woo-grid-new-badge';
	}

	public function get_title() {
		return __( '"New" Badge (Product Grid)', 'rey-core' );
	}
	...
	protected function element_register_controls() {
		...
		$this->start_controls_section(
			...
			$this->add_control(
				'newness',
				[
					'label' => esc_html__( 'Display duration (days)', 'rey-core' ),
		...
		$this->start_controls_section(
			...
			$selectors['main'] = '{{WRAPPER}} .rey-new-badge';
	...
	public function render_template() {
		...
		if( !( $component = \ReyCore\Plugin::instance()->woocommerce_loop->get_component('new_badge') ) ){
			return;
		}

		add_filter('theme_mod_loop_new_badge', [$this, '__enable']);
		add_filter('reycore/woocommerce/loop/new_badge/newness', [$this, '__newness']);
		add_filter('reycore/woocommerce/loop/new_text', [$this, '__text']);

			$component->render();

		remove_filter('reycore/woocommerce/loop/new_badge/newness', [$this, '__newness']);
		remove_filter('reycore/woocommerce/loop/new_text', [$this, '__text']);
		remove_filter('theme_mod_loop_new_badge', [$this, '__enable']);

	}

	public function __newness($val){

		if( $newness = $this->_settings['newness'] ){
			return $newness;
		}
