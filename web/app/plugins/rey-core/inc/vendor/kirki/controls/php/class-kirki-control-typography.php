...
class Kirki_Control_Typography extends Kirki_Control_Base {
	...
	public function to_json() {
		...
		// error_log(var_export( $this->json['value'], true));
		...
		if ( is_array( $this->json['value'] ) ) {
			foreach ( array_keys( $this->json['value'] ) as $key ) {
				...
				if (
					...
					|| (isset( $this->json['default'][ $key ] ) && false === $this->json['default'][ $key ])
				) {
					unset( $this->json['value'][ $key ] );
				}
		...
		<div class="wrapper">
			...
			<# if ( ! _.isUndefined( data.default['font-size'] ) ) { #>
				...
				<div class="font-size">
					...
					<input {{{ data.inputAttrs }}} type="text" value="{{ data.value['font-size'] }}" placeholder="eg: 14px"/>
				</div>
			<# } #>
			...
			<# if ( ! _.isUndefined( data.default['line-height'] ) ) { #>
				...
				<div class="line-height">
					...
					<input {{{ data.inputAttrs }}} type="text" value="{{ data.value['line-height'] }}" placeholder="eg: 1.5"/>
				</div>
			<# } #>
			...
			<# if ( ! _.isUndefined( data.default['letter-spacing'] ) ) { #>
				...
				<div class="letter-spacing">
					...
					<input {{{ data.inputAttrs }}} type="text" value="{{ data.value['letter-spacing'] }}" placeholder="eg: 0px"/>
				</div>
			<# } #>
