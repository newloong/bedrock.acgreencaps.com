		...
		webfonts: {
			...
			google: {
				...
				setFonts: function() {
					...
					self.fonts = kirkiL10n.googleFontList;
				...
				getFonts: function( order, category, number ) {
					...
					ordered = jQuery.extend( {}, self.fonts );
			...
			standard: {
				...
				setFonts: function() {
					...
					self.fonts = kirkiL10n.standardFontList;
				},
			...
			getFontType: function( family ) {
				...
				if ( 'undefined' !== typeof self.google.fonts[ family ] ) {
					return 'google';
				}
				return false;
			}
...
wp.customize.controlConstructor['kirki-typography'] = wp.customize.kirkiDynamicControl.extend( {
	...
	initKirkiControl: function() {
		...
		this.fontObject = {};
		this.familySelector = this.selector + ' .font-family select';

		this.createFontList();
		this.renderFontSelector();
		this.renderBackupFontSelector();
	...
	createFontList: function(){
		...
		var standardFonts = [];
		var googleFonts   = [];
		var fonts         = control.getFonts();
		var controlFontFamilies = {};
		...
		if (
			! _.isUndefined( control.params ) &&
			! _.isUndefined( control.params.choices ) &&
			! _.isUndefined( control.params.choices.fonts ) &&
			! _.isUndefined( control.params.choices.fonts.families )
		) {
			controlFontFamilies = control.params.choices.fonts.families;
		}

		var extraFonts = JSON.parse( JSON.stringify( kirkiL10n.extraFontListGroups ) || "{}" );

		if(
			typeof extraFonts.custom == 'object' &&
			typeof extraFonts.custom.children == 'object' &&
			( control.params.id === "typography_primary" || control.params.id === "typography_secondary" )
		){

			var newChildrenObject = [];

			Object.keys(extraFonts.custom.children).forEach(i => {
				if( ! (
					'var(--primary-ff)' === extraFonts.custom.children[i]['id'] ||
					'var(--secondary-ff)' === extraFonts.custom.children[i]['id']
				) ){
					newChildrenObject.push(extraFonts.custom.children[i]);
				}
			});

			extraFonts.custom.children = newChildrenObject;
		}
		...
		// window.reyCustomizerFonts = _.values( data );
		this.fontObject = _.values( data );

	},

	updateFontList: function(){

		if( ! ("typography_primary" === this.id || "typography_secondary" === this.id) ){
			return;
		}

		var types = {
			'typography_primary': 'Primary Font (%s)',
			'typography_secondary': 'Secondary Font (%s)',
		};

		var newValue = types[ this.id ].replace( /%s/g, this.setting._value['font-family'] );

		wp.customize.control.each( control => {

			if (control.params.type !== "kirki-typography") {
				return;
			}

			if (control.id === "typography_primary" || control.id === "typography_secondary") {
				return;
			}

			// console.log(control.fontObject);

			// this.fontObject.forEach(element => {
			// 	console.log(element);
			// });

			// this.fontObject

			// this.fontSelect = jQuery( selector ).selectWoo( {
			// 	data: this.fontObject,
	...
	renderFontSelector: function() {
		...
		this.fontSelect = jQuery( this.familySelector ).selectWoo( {
			data: this.fontObject,
			dropdownParent: jQuery(this.familySelector).parent(),
		} );

		// Set the initial value.
		if ( value['font-family'] || '' === value['font-family'] ) {
			value['font-family'] = kirki.util.parseHtmlEntities( value['font-family'].replace( /'/g, '"' ) );
			value['font-family'] = value['font-family'].replace( /__/g, '"' );
			this.fontSelect.val( value['font-family'] ).trigger( 'change' );
		}
		...
		this.fontSelect.on( 'change', function() {
			...
			// update font list for primary & secondary
			// control.updateFontList();
		} );
	...
	renderVariantSelector: function() {
		...
		var control    = this,
			...
			variants   = kirkiL10n.fontsVariants,
			fontWeight,
			variantSelector,
			fontStyle;
