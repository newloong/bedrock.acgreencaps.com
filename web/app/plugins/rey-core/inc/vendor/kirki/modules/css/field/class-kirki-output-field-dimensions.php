...
class Kirki_Output_Field_Dimensions extends Kirki_Output {
	...
	protected function process_output( $output, $value ) {
		...
		foreach ( array_keys( $value ) as $key ) {
			...
			//! REY
			$unit = isset($output['units']) ? $output['units'] : '';
			$val = $value[ $key ] . ( is_numeric($value[ $key ]) ? $unit : '' );

			$this->styles[ $output['media_query'] ][ $output['element'] ][ $property ] = $output['prefix'] . $this->process_property_value( $property, $val ) . $output['suffix'];
		}
	}
}
