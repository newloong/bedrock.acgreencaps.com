...
class Breadcrumb {
	...
	public function generate() {
		...
		if ( ( ! is_front_page() && ! ( is_post_type_archive() && intval( get_option( 'page_on_front' ) ) === rey__wc_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	private function prepend_shop_page() {
		$permalinks   = rey__wc_get_permalink_structure();
		$shop_page_id = rey__wc_get_page_id( 'shop' );
	...
	private function add_crumbs_single( $post_id = 0, $permalink = '' ) {
		...
		if ( 'product' === get_post_type( $post ) ) {
			...
			$terms = rey__wc_get_product_terms(
	...
	private function add_crumbs_shop() {
		if ( intval( get_option( 'page_on_front' ) ) === rey__wc_get_page_id( 'shop' ) ) {
			return;
		}

		$_name = rey__wc_get_page_id( 'shop' ) ? get_the_title( rey__wc_get_page_id( 'shop' ) ) : '';
	...
	private function endpoint_trail() {
		$endpoint       = rey__is_wc_endpoint_url() ? WC()->query->get_current_endpoint() : '';
...
if( !function_exists('rey__wc_get_page_id') ){
	function rey__wc_get_page_id( $page ) { return false; }
}
if( !function_exists('rey__is_wc_endpoint_url') ){
	function rey__is_wc_endpoint_url() { return false; }
}
if( !function_exists('rey__wc_get_permalink_structure') ){
	function rey__wc_get_permalink_structure() { return false; }
}
if( !function_exists('rey__is_product_category') ){
	function rey__is_product_category() { return false; }
}
if( !function_exists('rey__is_product_tag') ){
	function rey__is_product_tag() { return false; }
}
if( !function_exists('rey__is_shop') ){
	function rey__is_shop() { return false; }
}
if( !function_exists('rey__wc_get_product_terms') ){
	function rey__wc_get_product_terms( $product_id, $taxonomy, $args = array() ) {
		return [];
	}
}
