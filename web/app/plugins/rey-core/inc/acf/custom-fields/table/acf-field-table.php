<?php
/**
 * Credits: Advanced Custom Fields: Table Field by Johann Heyne
 * http://www.johannheyne.de/
 */
class ReyAcfFieldTable extends acf_field {
	...
	function __construct() {
		...
		*/
		...
		$this->label = _x('Table', 'Acf table field', 'rey-core');
				...
				add_filter( 'update_post_metadata', function( $x, $object_id, $meta_key, $meta_value, $prev_value ) {
					...
					if (
						...
						if ( json_last_error() !== JSON_ERROR_NONE ) {
							...
							error_log( 'The ACF Table Field prevented a third party update_post_meta( ' . $object_id . ', "' . $meta_key . '", $value ); action that would save a broken JSON string.' . "\n" . 'For details see https://codex.wordpress.org/Function_Reference/update_post_meta#Character_Escaping.' );
							return true;
						}
					}
	...
	function render_field_settings( $field ) {
		...
		*/
		...
		acf_render_field_setting( $field, array(
			'label'			=> _x('Table Header', 'Acf table field','rey-core'),
			'instructions'	=> _x('Presetting the usage of table header', 'Acf table field','rey-core'),
			'type'			=> 'radio',
			'name'			=> 'use_header',
			'choices'   	=>  array(
				0   =>  _x( "Optional", 'Acf table field', 'rey-core' ),
				1   =>  _x( "Yes", 'Acf table field', 'rey-core' ),
				2   =>  _x( "No", 'Acf table field', 'rey-core' ),
			),
			'layout'		=>  'horizontal',
			'default_value'	=> 0,
		));
		...
		acf_render_field_setting( $field, array(
			'label'			=> _x('Table Caption', 'Acf table field','rey-core'),
			'instructions'	=> _x('Presetting the usage of table caption', 'Acf table field','rey-core'),
			'type'			=> 'radio',
			'name'			=> 'use_caption',
			'choices'   	=>  array(
				1   =>  _x( "Yes", 'Acf table field', 'rey-core' ),
			...
			$e .= '<div class="acf-table-optionwrap">';
				...
				// OPTION HEADER {
					...
					if ( $data_field['use_header'] === 0 ) {
						...
						$e .= '<div class="acf-table-optionbox">';
							$e .= '<label for="acf-table-opt-use-header">' . _x( 'use table header', 'Acf table field', 'rey-core' ) . ' </label>';
							$e .= '<select class="acf-table-optionbox-field acf-table-fc-opt-use-header" id="acf-table-opt-use-header" name="acf-table-opt-use-header">';
								$e .= '<option value="0">' . _x( 'No', 'Acf table field', 'rey-core' ) . '</option>';
								$e .= '<option value="1">' . _x( 'Yes', 'Acf table field', 'rey-core' ) . '</option>';
							$e .= '</select>';
						$e .= '</div>';
					}
				...
				// OPTION CAPTION {
					...
					if ( $data_field['use_caption'] === 1 ) {
						...
						$e .= '<div class="acf-table-optionbox">';
							$e .= '<label for="acf-table-opt-caption">' . _x( 'table caption', 'Acf table field', 'rey-core' ) . ' </label><br>';
...
// initialize
acf_register_field_type( new ReyAcfFieldTable() );
